...
export const useBodyActions = ({
  ...
  const manageTimelineContext = useTimelineActions({
    filterManager,
    isQueryLoading: loading,
    type: timelineTypeContext,
  });
  console.log('manageTimelineContext useBodyActions', manageTimelineContext);
  ...
  const onRowSelected: OnRowSelected = useCallback(
    ({ eventIds, isSelected }: { eventIds: string[]; isSelected: boolean }) => {
      setSelected!({
        id,
        eventIds: getEventIdToDataMapping(data, eventIds, manageTimelineContext.queryFields ?? []),
        isSelected,
        isSelectAllChecked: isSelected && Object.keys(selectedEventIds).length + 1 === data.length,
      });
    },
    [setSelected, id, data, selectedEventIds, manageTimelineContext.queryFields]
  );
      ...
      isSelected
        ? setSelected!({
            ...
            eventIds: getEventIdToDataMapping(
              ...
              manageTimelineContext.queryFields ?? []
            ),
            isSelected,
            isSelectAllChecked: isSelected,
          })
        : clearSelected!({ id }),
    [setSelected, clearSelected, id, data, manageTimelineContext.queryFields]
  );
  ...
  // Sync to manageTimelineContext.selectAll so parent components can select all events
  useEffect(() => {
    if (manageTimelineContext.selectAll) {
      onSelectAll({ isSelected: true });
    }
  }, [manageTimelineContext.selectAll]); // onSelectAll dependency not necessary
